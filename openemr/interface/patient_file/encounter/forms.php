...
<?php
  if ($result = getFormByEncounter($pid, $encounter, "id, date, form_id, form_name, formdir, user, deleted")) {
	...
	$do_esign = true;
	if ($formdir == 'laboratory' || $formdir == 'labcorp' || $formdir == 'quest') $do_esign = false;
        ...
        if ( $esign->isButtonViewable() && $do_esign) {
            echo $esign->buttonHtml();
        }
        ...
        if ( $esign->isLogViewable() && $do_esign ) {
            $esign->renderLog();
        }
