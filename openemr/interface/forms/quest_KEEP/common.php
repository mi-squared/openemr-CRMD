...
try {
	...
	if ($order_data->id && strtotime($order_data->order_datetime)) $order_date = date('Y-m-d',strtotime($order_data->order_datetime));
	if ($order_data->user == 'system') $generated = true;
	if (! $pid && $order_data->patient_id) $pid = $order_data->patient_id;
...
if (!$siteid) $siteid = $lab_data['send_fac_id'];
...
if (!$lab_data["login"]) $invalid .= "No LabCorp Username\n";
if (!$lab_data["password"]) $invalid .= "No LabCorp Password\n";
if (!$lab_data["orders_path"]) $invalid .= "No LabCorp Order Path\n";
if (!$lab_data["results_path"]) $invalid .= "No LabCorp Result Path\n";
				...
				$.ajax ({
					...
					success: function(data) {
				    	$.each(data, function(key, val) {
				    		...
				    		output += "<tr><td style='white-space:nowrap'><input class='wmtCheck' type='checkbox' name='check_"+id+"' code='"+val.code+"' desc='"+val.long_desc+"'/> <b>"+val.code+"</b> - </td><td style='width:auto;text-align:left'>"+val.short_desc+"<br/></td>\n";
						});
					},
					async:   false
				});
				...
				$.ajax ({
					...
					success: function(data) {
				    	...
				    	$.each(data, function(key, val) {
				    		...
				    		output += " - </td><td style='width:auto;text-align:left'>"+val.title+"<br/></td>\n";
				    	});
					},
					async:   false
				});
...
<?php } ?>
				...
				newRow += "<input type='hidden' name='test_type[]' value='"+type+"' />\n";
				...
				for (var key in aoe) {
					...
					if (test_code) {
						newRow += '<input type="hidden" name="aoe'+id+'_label[]" value="'+question+'" />'+"\n";
						newRow += "<input type='hidden' name='aoe"+id+"_code[]' value='"+test_code+"' />\n";
						newRow += "<div style='margin-top:5px'>" + question + ": <input name='aoe"+id+"_text[]' title='" + prompt + "' class='wmtFullInput aoe' value='' style='width:300px' /></div>\n";
					}	
				}
...
<?php if ($viewmode) { ?>
				...
				<span class=title><?php echo $form_title; ?> <?php echo (strtotime($order_data->date_transmitted))? '&amp; Results View': 'Update' ?></span>
<?php } else { ?>
...
<!-- BEGIN ORDER -->
			...
			<!-- Client Information -->
			<div class="wmtMainContainer" id="clientData" style="width:99%">
				<div class="wmtCollapseBar" id="ReviewCollapseBar" style="border-bottom:none" onclick="togglePanel('ReviewBox','ReviewImageL','ReviewImageR','ReviewCollapseBar')">
					<table style="width:100%">	
						<tr>
							<td>
								<img id="ReviewImageL" align="left" src="<?php echo $webroot;?>/library/wmt/fill-270.png" border="0" alt="Show/Hide" title="Show/Hide" />
							</td>
							<td class="wmtChapter" style="text-align: center">
								Patient Information
							</td>
							<td style="text-align: right">
								<img id="ReviewImageR" src="<?php echo $webroot;?>/library/wmt/fill-270.png" border="0" alt="Show/Hide" title="Show/Hide" />
							</td>
						</tr>
					</table>
				</div>
				
				<div class="wmtCollapseBox" id="ReviewBox" style="display:none">
					<table style="width:100%">	
						<tr>
							<!-- Left Side -->
								...
								<table style="width:99%">
									...
									<tr>
										<td colspan="3" class="wmtLabel">
											...
											<input id="ins_secondard" name="ins_secondary" type="hidden" value="<?php echo $ins_list[1]->id ?>"/>
										</td>
			...
			<!--  Order Entry -->
			<div class="wmtMainContainer" id="orderEntry" style="width:99%;<?php if ($status != 'i') echo 'display:none'; ?>">
				...
				<div class="wmtCollapseBox" id="EntryBox">
					<table style="width:100%">
						<tr>
							<!-- Left Side -->
							<td class="wmtInnerLeft" style='width:49%;padding-left:5px;padding-right:4px'>
							        ...
							        <tr>
										<td class="wmtLabHeader">
											...
											<div style="float:left;vertical-align:bottom;">
												<input class="wmtButton" type="button" onclick="addCodes()" value="add selected"/>
											</div>
											<div style="float:right">
												<input class="wmtInput" style="barckground-color:white" type="text" name="searchIcd" id="searchIcd" />
...
foreach ($dlist as $data) {
	...
	echo "<tr><td style='white-space:nowrap;width:60px'><input class='wmtCheck' type='checkbox' id='check_".$id."' code='".$data['code']."' desc='".htmlspecialchars($text)."' > <b>".$data['code']."</b></input> - </td><td style='padding-top:0'>".$text."</td></tr>\n";
}
								...
								<table class="wmtLabBox">
							        <tr>
										<td class="wmtLabHeader">
											...
											<div style="float:left;vertical-align:bottom;">
												<input class="wmtButton" type="button" onclick="addTests()" value="add selected"/>
											</div>
											<div style="float:right">
												<input class="wmtInput" style="background-color:white" type="text" name="searchCode" id="searchCode" />
...
?>
			...
			<!--  Order Review -->
			<div class="wmtMainContainer" id="orderReview" style="width:99%">
				<div class="wmtCollapseBar" id="OrderCollapseBar" style="<?php if ($status != 'i') echo "border-bottom:none" ?>" onclick="togglePanel('OrderBox','OrderImageL','OrderImageR','OrderCollapseBar')">
					<table style="width:100%">	
						<tr>
							<td>
...
<?php if ($ins_list[0]->plan_type == 2) { // medicare ?>
									<table style="margin-bottom:10px">
										<tr>
											<td colspan="8">
												<input type="checkbox" class="wmtCheck" id="order_abn_signed" name="order_abn_signed" value="Y"	<?php if ($order_data->order_abn_signed) echo "checked" ?> />
									...
									<table style="margin-bottom:10px">
										...
										<tr id="ship_data" style="<?php if ($order_data->order_psc || (!$viewmode && $GLOBALS['wmt_lab_psc'])) echo "display:none" ?>">
											<td>
												...
												<input type="text" class="wmtInput" id="specimen_transport" name="specimen_transport" readonly style="width:220px" value="<?php echo $order_data->specimen_transport ?>" />
											</td>
										</tr>	
										<tr id="sample_data" style="<?php if ($order_data->order_psc || (!$viewmode && $GLOBALS['wmt_lab_psc'])) echo "display:none" ?>">
											<td style='width:90px'>
												...
												<input class="wmtInput" type='text' size='12' name='specimen_date' id='specimen_date' 
													value='<?php echo $viewmode ? ($order_data->date_collected == 0)? '' : date('Y-m-d',strtotime($order_data->date_collected)) : date('Y-m-d'); ?>'
													title='<?php xl('yyyy-mm-dd Date sample taken','e'); ?>'
													onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' />
												<img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22'
													id='img_specimen_date' border='0' alt='[?]' style='cursor:pointer;cursor:hand;<?php if ($order_data->order_psc) echo "display:none" ?>'
													title='<?php xl('Click here to choose a date','e'); ?>'>
											</td>
											<!-- td style='text-align:right;width:40px'>
												<label class="wmtLabel">Time: </label>
											</td><td>
												<input type="text" class="wmtInput" style="width:65px" name='specimen_time' id='specimen_time' <?php if ($order_data->order_psc) echo "disabled " ?>
												value='<?php echo $viewmode ? (strtotime($order_data->date_collected) === false)? '' : date('h:ia',strtotime($order_data->date_collected)) : date('h:ia'); ?>' />
											</td -->
<?php if ($lab_data['npi'] != 'QUEST' && $lab_data['recv_fac_id'] != 'QBA') { ?>
											<td style="padding-left:25px">
												<label class="wmtLabel">Volume: </label>
											</td><td style="white-space:nowrap">
												<input type="text" class="wmtInput" style="width:65px" name='specimen_volume' id='specimen_volume' <?php if ($order_data->order_psc) echo "disabled " ?>
...
<?php } // END SPECIAL FOR QBA ?>
											...
											<td style='text-align:right;width:130px'>
												...
												<input type="input" name="specimen_duration" class="wmtInput" style="width:65px" value='<?php echo $order_data->specimen_duration; ?>'/>
											</td>
											
											<!-- td style="padding-left:25px;white-space:nowrap">
												<label class="wmtLabel" style="vertical-align:middle">Specimen Location:&nbsp;&nbsp;</label>
												<select name='specimen_location' class='wmtSelect'>
													<?php ListSel($order_data->specimen_location,'proc_body_site') ?>
												</select>
											</td>
											...
											<td>
												<label class="wmtLabel">Specimen Type: &nbsp;&nbsp;</label>
												<select name='specimen_type' id='specimen_type' class='wmtSelect' readonly>
													<option value=""></option>
													<?php ListSel($order_data->specimen_type,'proc_specimen') ?>
												</select>
											</td -->
										</tr>
									
										<tr id="psc_data" style="<?php if ((!$order_data->order_psc && $viewmode) || !$GLOBALS['wmt_lab_psc']) echo "display:none" ?>">
											<td>
												<label class="wmtLabel" >Scheduled Date: </label>
											</td><td>
												<input class="wmtInput" type='text' size='12' name='pending_date' id='pending_date' 
													value='<?php echo $viewmode ? ($order_data->order_pending == 0)? '' : date('Y-m-d',strtotime($order_data->order_pending)) : ''; ?>'
													title='<?php xl('yyyy-mm-dd Date sample scheduled','e'); ?>'
													onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' />
												<img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22'
													id='img_pending_date' border='0' alt='[?]' style='cursor:pointer;cursor:hand'
													title='<?php xl('Click here to choose a date','e'); ?>'>
											</td>
...
foreach ($item_list as $order_item) { // $item = array of objects
	...
	if ($order_item->procedure_type == 'pro') {
		$newRow .= "style='font-weight:bold;color:#c00' /><input type='hidden' name='test_profile[]' value='P' />";
	}
	else {
		$newRow .= "style='font-weight:bold' /><input type='hidden' name='test_profile[]' value='S' />";
	} 
	...
	while ($aoe = sqlFetchArray($result)) {
		...
		if ($aoe['code']) {
			...
			$newRow .= "<div style='margin-top:5px'>".$question.": <input name='aoe".$aoe['code']."_text[]' title='".$aoe['tips']."' class='wmtFullInput aoe' value='".$aoe['answer']."' style='width:300px' /></div>\n";
			$aoe_count++;
		}
	}
...
<?php } ?>
									...
									<table style="width:100%">
										<tr>
											<td>
												<label class="wmtLabel">Notes / Clinical History:</label>
			...
			<!-- Order Submission -->
			<div class="wmtMainContainer" id="orderSubmission" style="width:99%;">
				<div class="wmtCollapseBar" id="InfoCollapseBar" style="<?php if ($status != 'i') echo "border-bottom:none" ?>" onclick="togglePanel('InfoBox','InfoImageL','InfoImageR','InfoCollapseBar')">
					<table style="width:100%">
						<tr>
									...
									<tr>
										...
										<td nowrap>
											<input class="wmtInput" type='text' size='10' name='date_ordered' id='date_ordered' 
												value='<?php echo $viewmode ? date('Y-m-d',strtotime($order_data->date_ordered)) : date('Y-m-d'); ?>'
										...
										<td>
											<select class="wmtSelect" name='provider_id' id='provider_id' style="min-width:150px">
...
<?php 
	...
	while ($rrow= sqlFetchArray($rlist)) {
		...
		if (!$order_data->provider_id && $_SESSION['authUserID'] == $rrow['username']) echo " selected";
...
?>
										...
										<td nowrap>
											<select class="wmtInput" name="request_account" style="width:150px" />
<?php 
...
?>
									...
									<tr>		
										...
										<td nowrap>
											<input class="wmtInput" readonly style="width:150px" value="<?php echo ($order_data->date_transmitted > 0)?date('Y-m-d H:i:s',strtotime($order_data->date_transmitted)):''?>" />
										</td>
									</tr>
...
<!-- START OF RESULTS -->
			...
			<!--  External Results -->
			<div class="wmtMainContainer" id="resultEntry" style="width:99%;<?php if ($status == 'i' || $lab_data['protocol'] == 'INT') echo "display:none" ?>">
				<div class="wmtCollapseBar" id="ExternalCollapseBar" onclick="togglePanel('ExternalBox','ExternalImageL','ExternalImageR','ExternalCollapseBar')">
					<table style="width:100%">	
						<tr>
							<td>
...
?>
										<tr>
											<td style="padding-bottom:10px">
												...
												<input type="text" class="wmtInput" readonly style="width:220px" value="<?php echo $order_data->control_id ?>" />
											</td>
										</tr>
<?php 
	}
?>										<tr>
											<td style='width:100px'>
												...
												<input class="wmtInput" type='text' size='10' readonly value='<?php echo ($order_data->date_transmitted == 0)? '' : date('Y-m-d',strtotime($order_data->date_transmitted)); ?>' />
											</td>
											<td style='text-align:right'>
												<label class="wmtLabel">Time: </label>
											</td><td>
												<input type="input" class="wmtInput" style="width:65px" readonly value='<?php echo ($order_data->date_transmitted == 0)? '' : date('h:ia',strtotime($order_data->date_transmitted)); ?>' />
											</td>
										</tr>
										<tr>
											<td style='width:100px'>
												...
												<input class="wmtInput" type='text' size='10' readonly value='<?php echo ($order_data->date_collected == 0)? '' : date('Y-m-d',strtotime($order_data->date_collected)); ?>' />
											</td>
											<td style='text-align:right'>
												<label class="wmtLabel">Time: </label>
											</td><td>
												<input type="input" class="wmtInput" style="width:65px" readonly value='<?php echo ($order_data->date_collected == 0)? '' : date('h:ia',strtotime($order_data->date_collected)); ?>' />
											</td>
										</tr>
										<tr>
											<td style='width:100px'>
												...
												<input class="wmtInput" type='text' size='10' readonly value='<?php echo ($order_data->result_datetime == 0)? '' : date('Y-m-d',strtotime($order_data->result_datetime)); ?>' />
											</td>
											<td style='text-align:right'>
												<label class="wmtLabel">Time: </label>
											</td><td>
												<input type="input" class="wmtInput" style="width:65px" readonly value='<?php echo ($order_data->result_datetime == 0)? '' : date('h:ia',strtotime($order_data->result_datetime)); ?>' />
											</td>
...
?>
										<tr <?php if ($abnormal) echo 'style="font-weight:bold;color:#bb0000"'?>>
											<td colspan="1">&nbsp;</td>
											<td colspan="8" style="padding-left:120px;text-align:left;font-family:monospace;">
...
?>
			...
			<!--  Result Review -->
			<div class="wmtMainContainer" id="resultReview" style="width:99%;<?php if ($status == 'i' || $status == 's') echo "display:none" ?>">
							...
							<td style="width:50%">
								<table style="width:100%">
									<tr>
										...
										<td nowrap>
											<input class="wmtInput nolock" type='text' size='10' name='reviewed_date' id='date_reviewed' 
												value='<?php echo ($order_data->reviewed_datetime > 0)? date('Y-m-d',strtotime($order_data->reviewed_datetime)) : ''; ?>'
												title='<?php xl('yyyy-mm-dd Date results reviewed','e'); ?>'
												onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' />
											<img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22' 
												id='img_date_reviewed' border='0' alt='[?]' style='cursor:pointer;cursor:hand'
												title='<?php xl('Click here to choose a date','e'); ?>'>
										</td>
										<td class="wmtLabel" nowrap style="text-align:right">Reviewed By: </td>
										<td>
											<select class="wmtInput nolock" name='reviewed_id' id='reviewed_id' style="min-width:200px" onchange="$('#date_reviewed').val('<?php echo date('Y-m-d') ?>')">
												<option value='_blank'>-- select --</option>
<?php 
	$rlist= sqlStatement("SELECT * FROM users WHERE authorized=1 AND active=1 AND npi != '' ORDER BY lname");
	while ($rrow= sqlFetchArray($rlist)) {
...
?>
									...
									<tr>
										...
										<td nowrap>
											<input class="wmtInput nolock" type='text' size='10' name='notified_date' id='date_notified' 
												value='<?php echo ($order_data->notified_datetime > 0)? date('Y-m-d',strtotime($order_data->notified_datetime)) : ''; ?>'
												title='<?php xl('yyyy-mm-dd Date patient notified of results','e'); ?>'
												onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' />
											<img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22' 
												id='img_date_notified' border='0' alt='[?]' style='cursor:pointer;cursor:hand'
												title='<?php xl('Click here to choose a date','e'); ?>'>
										</td>
										<td class="wmtLabel" nowrap style="text-align:right">Notified By: </td>
										<td>
											<select class="wmtInput nolock" name='notified_id' id='notified_id' style="min-width:200px" onchange="$('#date_notified').val('<?php echo date('Y-m-d') ?>')">
												<option value='_blank'>-- select --</option>
<?php 
...
?>
									...
									<tr>
										...
										<td colspan='3'>
											<input type='text' id='notified_person' name='notified_person' class="wmtInput" style="width:300px" value="<?php echo $order_data->notified_person ?>" />
										</td>
									</tr>
							...
							<td>
								<table style="width:100%">
									<tr>
										<td class="wmtLabel" colspan="3">
											Review Notes: 
											<textarea name="review_notes" id="review_notes" class="wmtFullInput nolock" rows="4"><?php echo htmlspecialchars($order_data->review_notes) ?></textarea>
										</td>
									</tr>
								</table>
							</td>
...
<?php } ?>
	...
	<script>
		...
		Calendar.setup({inputField:"pending_date", ifFormat:"%Y-%m-%d", button:"img_pending_date"});
		Calendar.setup({inputField:"specimen_date", ifFormat:"%Y-%m-%d", button:"img_specimen_date"});
