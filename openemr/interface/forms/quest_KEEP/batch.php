...
require_once("{$GLOBALS['srcdir']}/wmt/quest/QuestParserHL7v23.php");
...
function labPnote($pid, $newtext, $assigned_to = '', $datetime = '') {
	if (!$pid || $pid > 999999990) return; // do not generate messages without a pid
...
$order_type = "quest";
$result_title = $lab_data['name']." Results - ";
...
//	if ($message->name[0] != 'TEST') continue; // DEBUGING
	...
	if ($message->pid) { // check pid
		$patient = sqlQuery("SELECT pid, DOB, providerID, lname, fname, mname FROM patient_data WHERE pid = ? and DOB = ?", array($message->pid, $pat_DOB) );
		if ($patient['pid']) $pid = $patient['pid'];
	}
	
	if (!$pid && $message->pubpid && $pat_DOB) { // maybe they used pubpid (require bdate too)
		$patient = sqlQuery("SELECT pid, DOB, providerID, lname, fname, mname FROM patient_data WHERE pubpid = ? and DOB = ?", array($message->pubpid, $pat_DOB) );
		if ($patient['pid']) {
			$pid = $patient['pid'];
			echo "NOTICE: PATIENT MATCHED USING PUBPID \n";
		}
	}
	...
	if (!$message->order_number) {
		echo "WARNING: ORDER NUMBER GENERATED FOR THESE RESULTS \n";
		$message->order_number = $GLOBALS['adodb']['db']->GenID('order_seq');
		$order_data = new wmtOrder($order_type); // create a dummy order
	}
	elseif (!$order_data->order_number) {
		echo "WARNING: NO MATCHING ORDER FOUND FOR THESE RESULTS \n";
	
		// watch out of order number assigned to different patient
		$dupchk = sqlQuery("SELECT id FROM form_quest WHERE order_number = ?",array($message->order_number));
		if ($dupchk['id']) {
	...
	if (!$facility_id) {
		...
		$site = sqlQuery("SELECT * FROM list_options WHERE list_id = 'Provider_Site_Identifier' AND title = ?",array($facility_id));
	...
	if (!$encounter && $pid != $DUMMY_ID) {
		...
		addForm($encounter, "QUEST RESULT ENCOUNTER",
			sqlInsert("INSERT INTO form_encounter SET " .
				...
				"onset_date = '', " .
				"reason = 'GENERATED ENCOUNTER FOR QUEST RESULT', " .
	...
	if (count($message->reports) > 0) { // do we have anything to process?
		...
		foreach ($message->reports as $report) {
			...
			$report_id = wmtResult::insert($report_data);
			...
			if (count($report->results) > 0) { // do we have results for this order?
				foreach ($report->results as $result) {
					...
					if ($DEBUG) {
						...
						echo "Observation Code: ".$result->observation_id[0]." \n";
						echo "Observation Text: ".$result->observation_id[1]." \n";
					...
					// generate the object
					$result_data = new wmtResultItem(); // empty object
					...
					$result_data->date = '';
					if (strtotime($result->observation_datetime))
					...
					$items[] = wmtResultItem::insert($result_data);
	...
	foreach ($message->documents as $document) {
		if ($document->documentData) {
			...
			while (file_exists($file_path.$file)) { // don't overlay duplicate file names
				$docName = $message->order_number . "_RESULT_".$docnum++;
				$file = $docName."_".$unique.".pdf";
			}
	...
	/* --------------------------------------------------------------------------- *
 	 ...
 	 * --------------------------------------------------------------------------- */
	$order_data->result_datetime = date('Y-m-d H:i:s');
	$order_data->result_abnormal = ($items_abnormal > 0)? 1 : 0;
	...
	if ($patient) {
		...
		$note .= "To review these results click on the following link: ";
  		$note .= "<a href='". $link_ref ."' target='_blank' class='link_submit' onclick='top.restoreSession()'>Quest Results - ". $message->order_number ." (". $message->lab_number .")</a>\n\n";
...
/*
	$query = "INSERT INTO procedure_batch SET ";
	$query .= "date = '$report_data->date_report', ";
	$query .= "pid = '$pid', ";
	$query .= "user = '$user', ";
	$query .= "groupname = 'system', ";
	$query .= "authorized = 0, ";
	$query .= "activity = 1, ";
	$query .= "facility_id = '$order_data->facility_id', ";
	$query .= "order_number = '$order_data->order_number', ";
	$query .= "order_datetime = '$order_data->date_ordered', ";
	$query .= "provider_id = '$order_data->provider_id', ";
	$query .= "provider_npi = '$doc_npi', ";
	$query .= "pat_dob = '$order_data->pat_DOB', ";
	$query .= "pat_first = '".mysql_real_escape_string($pat_fname)."', ";
	$query .= "pat_middle = '".mysql_real_escape_string($pat_mname)."', ";
	$query .= "pat_last = '".mysql_real_escape_string($pat_lname)."', ";
	$query .= "lab_number = '$report_data->specimen_num', ";
	$query .= "lab_status = '$report_data->report_status', ";
	$query .= "result_output = '$message->hl7data' ";
