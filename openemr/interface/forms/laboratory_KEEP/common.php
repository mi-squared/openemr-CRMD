				...
				$.ajax ({
					...
					success: function(data) {
				    	$.each(data, function(key, val) {
				    		...
				    		output += "<tr><td style='white-space:nowrap;width:60px'><input class='wmtCheck' type='checkbox' name='check_"+id+"' code='"+val.code+"' desc='"+val.long_desc+"'/> <b>"+code+"</b> - </td><td>"+val.short_desc+"<br/></td>\n";
						});
					},
					async:   false
				});
...
foreach ($dlist as $data) {
	...
	echo "<tr><td style='white-space:nowrap;width:60px'><input class='wmtCheck' type='checkbox' id='check_".$id."' code='".$data['code']."' desc='".htmlspecialchars($text)."' > <b>".$data['code']."</b></input> - </td><td style='padding-top:6px'>".$text."</td></tr>\n";
}
...
?>
							...
							<!-- Right Side -->
							<td  class="wmtInnerRight" style='width:49%;padding-left:7px;padding-right:3px'>
								<table class="wmtLabBox wmtColorBox">
							        <tr>
										<td class="wmtLabHeader wmtColorHeader">
											<div class="wmtLabTitle">
								...
								<fieldset>
									...
									<table id="codeTable" style="width:100%">
										<tr>
											...
											<th class="wmtHeader" style="width:110px">ICD Code</th>
...
<?php } ?>
										...
										<tr id="sample_data" style="<?php if ($order_data->order_psc || (!$viewmode && $GLOBALS['wmt_lab_psc']) || $lab_data['npi'] == 'CERNER') echo "display:none" ?>">
											...
											<td style='text-align:right;width:40px;white-space:nowrap'>
...
<?php if ($lab_data['npi'] != 'CERNER') { ?>
											<td style="text-align:right">
												...
												<input type="text" class="wmtInput" style="width:95px" name='specimen_source' id='specimen_source'
...
<?php } // end CERNER exclusion ?>
											<td style="text-align:right">
												...
												<input type="text" class="wmtInput" style="width:50px" name='specimen_volume' id='specimen_volume'
											...
											<!-- td style='text-align:right'>
												<label class="wmtLabel">Duration: </label>
											</td><td style="whitespace:nowrap">
												<input type="input" name="specimen_duration" class="wmtInput" style="width:65px" value='<?php echo $order_data->specimen_duration; ?>'/> <small>( hous )</small>
											</td -->
									...
									<tr>
										...
										<td>
											<select class="wmtSelect" name='provider_id' id='provider_id' style="min-width:150px;max-width:200px">
												<option value='_blank'>-- select provider --</option>
<?php 
...
?>
									...
									<tr>		
										...
										<td nowrap>
											<input class="wmtInput" readonly style="width:142px" value="<?php echo (!goodDate($order_data->date_transmitted))? '' : date('Y-m-d H:i:s',strtotime($order_data->date_transmitted));?>" />
										 -->
...
?>										<tr>
											<td style='width:100px'>
												...
												<input class="wmtInput" type='text' size='10' readonly value='<?php echo (!goodDate($order_data->date_transmitted))? '' : date('Y-m-d',strtotime($order_data->date_transmitted)); ?>' />
											</td>
											<td style='text-align:right;width:80px'>
												<label class="wmtLabel">Time: </label>
											</td><td>
												<input type="input" class="wmtInput" style="width:65px" readonly value='<?php echo (!goodDate($order_data->date_transmitted))? '' : date('h:ia',strtotime($order_data->date_transmitted)); ?>' />
											</td>
										</tr>
										<tr>
											<td style='width:100px'>
...
?>
										<tr style="vertical-align:baseline;<?php if ($abnormal) echo 'font-weight:bold;color:#bb0000'?>">
											<td>&nbsp;</td>
											<td>
...
?>
											<td colspan='4' style="font-family:monospace;text-align:left">
												<?php if ($result_data->result != "." && $result_data->result_data_type != 'FT') echo nl2br($result_data->result) ?>
											</td>
<?php 
					}
?>
									...
									<tr>
										...
										<td nowrap>
											<input class="wmtInput nolock" type='text' size='10' name='reviewed_date' id='date_reviewed' 
												value='<?php echo (!goodDate($order_data->reviewed_datetime))? '' : date('Y-m-d',strtotime($order_data->reviewed_datetime)); ?>'
									...
									<tr>
										...
										<td nowrap>
											<input class="wmtInput nolock" type='text' size='10' name='notified_date' id='date_notified' 
												value='<?php echo (!goodDate($order_data->notified_datetime))? '' : date('Y-m-d',strtotime($order_data->notified_datetime)); ?>'
...
<?php } ?>
	...
	<script>
		...
		Calendar.setup({inputField:"date_notified", ifFormat:"%Y-%m-%d", button:"img_date_notified"});
		Calendar.setup({inputField:"date_reviewed", ifFormat:"%Y-%m-%d", button:"img_date_reviewed"});
