...
// GET LAB DATA
if ($LAB) {
	$lab_data = sqlQuery("SELECT * FROM procedure_providers WHERE type = 'laboratory' AND ppid = ?",array($LAB));
	...
	* --------------------------------------------------------------------------- */
	...
	if ($message->labs) {
		foreach ($message->labs AS $lab) {
			...
			$old = sqlQuery("SELECT name, phone FROM procedure_facility WHERE code = ? AND lab_id = ?",$unique_code,$lab_id);
			while ($old && $old['name'] != $lab->name && $old['phone'] != $lab_phone) {
				$unique_code++;
				$old = sqlQuery("SELECT zip, phone FROM procedure_facility WHERE code = ? AND lab_id = ?",$unique_code,$lab_id);
			}
	...
	if (!$encounter && $pid != $DUMMY_ID) {
		...
		addForm($encounter, "LAB RESULT ENCOUNTER",
			...
			if (count($report->results) > 0) { // do we have results for this order?
				foreach ($report->results as $result) {
					...
					$results = sqlQuery("SELECT procedure_result_id FROM procedure_result WHERE procedure_report_id = ? AND result_code = ?",
							array($report_id,$result->observation_id[0]));
