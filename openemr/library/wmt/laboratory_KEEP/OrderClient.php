...
if (!class_exists("OrderClient")) {
	...
	class OrderClient {
		...
		public function buildRequest(&$order_data) {
			...
			if ($this->NPI == '1194769497' || $this->NPI == 'CPL') // Clinical Pathology Laboratories
		...
		public function addInsurance($setid, $key) {
			...
			if ($ins_data->subscriber_relation == 'Self') $relation = '1'; 
			if ($ins_data->subscriber_relation == 'Spouse') $relation = '2';
			...
			if ($this->NPI == 'PATHGROUP' || $this->NPI == '1235186800') {
				$relation = strtoupper($ins_data->subscriber_relation);
				if ($relation != 'SELF' && $relation != 'SPOUSE' && $relation != 'CHILD') $relation = 'OTHER';
			}

			$ins_identifier = $ins_data->id;
			if ($this->NPI == 'BIOREF') {
