...
if (!class_exists("QuestOrderClient")) {
	...
	class QuestOrderClient {
		...
		public function buildRequest(&$order_data) {
			...
			$this->request->hl7Order = sprintf($MSH, $this->SENDING_APPLICATION, $this->SENDING_FACILITY, $recvr, $this->RECEIVING_FACILITY, $trans);
			if ($this->DEBUG) echo $this->request->hl7Order . "\n"; // DEBUG
				
			// patient segment
			$PID = "PID|1||$patient_data->pubpid|$patient_data->pid|$pname||$dob|$sex||$patient_data->race|$paddress||$patient_data->phone_home|||||$order_data->account^^^$order_data->request_billing^$order_data->abn_signed|$patient_data->ss||||$patient_data->ethnicity|\r";
