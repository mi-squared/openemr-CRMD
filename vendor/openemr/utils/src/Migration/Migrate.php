...
require_once __DIR__."/../../../../../openemr/library/sql_upgrade_fx.php";
...
class Migrate
{
    ...
    public function insertMigrations( $migrations, $batch )
    {
        $statement = "INSERT INTO `migrations` ( `migration`, `batch` ) ";
        $values = array();
        $count = 0;
        foreach ( $migrations as $migration ) {
            $statement .= "VALUES ( ?, ? ) ";
            if ( $count < count( $migrations ) - 1 ) {
                $statement .= ", ";
            }
            $values[]= $migration;
            $values[]= $batch;
        ...
        return sqlInsert( $statement, $values );
    }
