...
if (empty($SBCODES)) {
  ...
  while ($prow = sqlFetchArray($pres)) {
    ...
    if ($prow['option_id'] == 'otherservice' or
        $prow['option_id'] == 'otherdiag' ) {
          $SBCODES[] = '*B|';
          $SBCODES[] = '*B|';
    }
...
function genColumn($ix) {
  global $SBCODES;
  // List of codes to add shading or something per td.fs-color-code
  $color_code = array(
                '99211',
                '99212',
                '99214',
  ...
  for ($imax = count($SBCODES); $ix < $imax; ++$ix) {
    ...
    if ($cmd == '*B') { // Borderless and empty
      ...
      if (count($b) <= 1) {
        ...
        $code = $b[0];
        if (!$code) $code='&nbsp;';
        if (in_array($code,$color_code)) {
            $fsclass = 'fs-color-code';
        }
        echo "     <td class='$fsclass' style='vertical-align:middle' nowrap>$code</td>\n";
        echo "     <td colspan='3' class='$fsclass' style='vertical-align:middle' nowrap>$title</td>\n";
      }
      else {
...
<body bgcolor='#ffffff'>
<form name='theform' method='post' action='routing_slip.php?fill=<?php echo $form_fill; ?>'
 onsubmit='return top.restoreSession()'>
<center>

<?php while (--$pages >= 0) { ?>

<?php echo genFacilityTitle(xl('Routing Sheet/SuperBill'), -1); ?>
 ...
 <tr>
  <td valign='top'>
   <table border='0' cellspacing='0' cellpadding='0' width='100%'>
    ...
    <tr>
     <td colspan='3' valign='top' class='fsgroup' style='height:<?php echo $lheight *2; ?>pt'> 
      Patient:<br />
<?php
if ($form_fill) {
...
?>
     ...
     <td valign='top' class='fshead'>
      DOB:&nbsp; <?php if ($form_fill) echo $patdata['DOB']; ?> <br /><br />
      ID:&nbsp;  <?php if ($form_fill) echo $patdata['pubpid']; ?> 
     </td>
    </tr>
    <tr>
     <td colspan='4' valign='top' class='fshead' style='height:<?php echo $lheight; ?>pt'>

<?php  // Use Current Date or Enc Date
    ...
    <tr>
     <td colspan='4' valign='top' class='fshead' style='height:<?php echo $lheight * 8; ?>pt'>
      ...
      <ul>
        ...
        <li>[&nbsp;&nbsp;] Weeks(s)</li>
        <li>[&nbsp;&nbsp;] Months(s)</li>
        <li>[&nbsp;&nbsp;] Specific Appt Date: ____________
      </ul>
      &nbsp;Payment Type: <br />
      &nbsp;[&nbsp;&nbsp;]-Cash &nbsp;&nbsp;[&nbsp;&nbsp;]-Check &nbsp;&nbsp;[&nbsp;&nbsp;]-Credit 
     </td>
...
<?php if ($pages == 0) { // if this is the last page ?>
    ...
    <tr>
     <td valign='top' colspan='4' class='fshead' style='height:<?php echo $lheight * 6; ?>pt;border-width:1 1px 0 0'>
      &nbsp;
     </td>
    </tr>
    <tr>
